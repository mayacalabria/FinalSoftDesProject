...
def update_search_term(attrname, old, new):
    ...
    #if frequency button is active create frequency plots
    if button_group2.active == 1:
        #if player button is active initialize player class
        if button_group1.active == 0:
            print(search_bar.value)
            #initialize class with search bar value
            player = map_classes.Player(search_bar.value)
            #get final and rookie seasons to set slider values
            season = player.final_season()
            rookie = player.rookie_season()
            final = player.final_season()
            index1 = int(rookie[0:4])
            index2 = int(final[0:4])
            #in case of rookies define 2 indexes so the slider doesn't break
            if index1==index2:
                index1 = index2-1
            ...
            slider.end = index2
            slider.value = index2
        #otherwise do similar things for team, but set slider values to include
        #all possible years
        else:
            print(search_bar.value)
            ...
            slider.end = 2017
            slider.value = 2017
    #same process but for accuracy button being active
    else:
        if button_group1.active == 0:
            print(search_bar.value)
            player = map_classes.Player(search_bar.value)
...
def update_plot_type(attrname, old, new):
    ...
    rootLayout = curdoc().get_model_by_name('mainLayout')
    listOfSubLayouts = rootLayout.children
    #if player button is active make player plots
    if button_group1.active == 0:
        #determine what type of plot is generated
        if button_group2.active == 1:
            #remove current plot
        ...
        #same process for other type of plot
        else:
            plotToRemove = curdoc().get_model_by_name('plot')
            listOfSubLayouts.remove(plotToRemove)
            player = map_classes.Player(search_bar.value)
            ...
            plotToAdd = p2
            listOfSubLayouts.append(plotToAdd)
    #same process as above for teams
    else:
        if button_group2.active == 1:
            plotToRemove = curdoc().get_model_by_name('plot')
            listOfSubLayouts.remove(plotToRemove)
...
def update_year(attrname, old, new):
    ...
    rootLayout = curdoc().get_model_by_name('mainLayout')
    listOfSubLayouts = rootLayout.children
    #if player button is active initialize player classes
    if button_group1.active == 0:
        #determine what type of plot is beign asked for and build that one
        if button_group2.active == 1:
            print (slider.value)
            #remove current plot
            plotToRemove = curdoc().get_model_by_name('plot')
            listOfSubLayouts.remove(plotToRemove)
            #initialize player class and get the updated year
            player = map_classes.Player(search_bar.value)
            ...
            plotToAdd = p2
            listOfSubLayouts.append(plotToAdd)
        #same as above with a different type of plot
        else:
            print (slider.value)
            plotToRemove = curdoc().get_model_by_name('plot')
            listOfSubLayouts.remove(plotToRemove)
